...
def TDM(factDict, claimDict, beta_set, iterNumber, burninNumber, thinNumber, ratio):
    ...
    return source_quality_matrix, timecost

def SRV(factDict, claimDict, SouceQualityMatrix, folderWrite, timecost, outputWritePath):
    def generate_fact_source_dict(claimDict):
        fact_claimed_source_dict = dict()
        fact_unclaimed_source_dict = dict()
        for key1 in claimDict.keys():
            if key1 not in fact_claimed_source_dict.keys():
                fact_claimed_source_dict[key1] = set()
            if key1 not in fact_unclaimed_source_dict.keys():
                fact_unclaimed_source_dict[key1] = set()
            for key2 in claimDict[key1].keys():
                if claimDict[key1][key2] == "1":
                    fact_claimed_source_dict[key1].add(key2)
                elif claimDict[key1][key2] == "0":
                    fact_unclaimed_source_dict[key1].add(key2)
        return fact_claimed_source_dict, fact_unclaimed_source_dict

    def generate_truth_dict(factDict, factClaimedSourceDict, factUnclaimedSourceDict, SouceQualityMatrix):
        truth_probability_dict = dict()
        for key1 in factDict.keys():
            true_probability = 1
            false_probability = 1
            for claimedSource in factClaimedSourceDict[key1]:
                true_probability = true_probability * SouceQualityMatrix[claimedSource]["recall"]
                false_probability = false_probability * (1 - SouceQualityMatrix[claimedSource]["recall"])
            for unclaimedSource in factUnclaimedSourceDict[key1]:
                true_probability = true_probability * (1 - SouceQualityMatrix[unclaimedSource]["specificity"])
                false_probability = false_probability * SouceQualityMatrix[unclaimedSource]["specificity"]
            truth_probability_dict[key1] = true_probability / (true_probability + false_probability)
        truth_dict = dict()
        for key1 in truth_probability_dict.keys():
            for key2 in factDict[key1].keys():
                if key2 not in truth_dict.keys():
                    truth_dict[key2] = list()
                if truth_probability_dict[key1] >= 0.4:
                    truth_dict[key2].append(key1)
        return truth_dict
    ...
    start = time.time()
    factClaimedSourceDict, factUnclaimedSourceDict = generate_fact_source_dict(claimDict)
    truthDict = generate_truth_dict(factDict, factClaimedSourceDict, factUnclaimedSourceDict, SouceQualityMatrix)
    dataWriteList = write_truth_dict(factDict, truthDict)
    used_time = time.time() - start + timecost
    result_write = open(file=outputWritePath + "/" + folderWrite + "/" + "truth_result_SRV_" + folderWrite, mode="w", encoding="utf-8")
    result_write.write("OID\tInferTruth\n")
    result_write.writelines(dataWriteList)
    result_write.close()
    result_write = open(file="./" + "journal_" + folderWrite, mode="a+", encoding="utf-8")
    result_write.write("Experiment on " + datetime.datetime.now().strftime('%Y_%m_%d_%H_%M') + "\n")
    result_write.write("Total Time : " + str(used_time) + "\n")
    result_write.close()

def main_for_SRV(dataset, datasetPath, outputWritePath):
    mapping = dict()
    mapping[1] = "movie"
    mapping[0] = "book"
    mapping[-1] = "test"
    ...
    sourceQualityMatrix, timecost = TDM(factDict, claimDict, beta_set, iterationNumber, burninNumber, thinNumber, ratio)
    SRV(factDict, claimDict, sourceQualityMatrix, mapping[dataset], timecost, outputWritePath)

#main_for_SRV(1, "../DataToUse", "./")
#main_for_SRV(0, "../DataToUse", "./")